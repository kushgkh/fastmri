...
def define_network_5_2chan(verbose=False):
    autoencoder = Sequential([
        Convolution2D(8, 3, padding = 'same', input_shape = (306, 304, 2)), 
        MaxPooling2D(padding='same'),
        #BatchNormalization(),
        #LeakyReLU(alpha=0.1),
        Activation('relu'), 
        ...
        ####
        Flatten(),
        Dense(((306 // (2**4)) * (304 // (2**4)) * 2), activation = 'tanh'),
        # Dense(192, input_shape = (encoding_size,), activation = 'relu'),
        Reshape(((306 // (2**4)), (304 // (2**4)), 2)),
        #BatchNormalization(),
        Convolution2D(128, 3, padding = 'same'),
        #BatchNormalization(),
        #LeakyReLU(alpha=0.1), #Activation('relu'),
